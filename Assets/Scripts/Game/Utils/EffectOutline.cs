...
[AddComponentMenu("UI/Effects/OutlineNew")]
public class EffectOutline : BaseMeshEffect
{
    protected EffectOutline()
    {
    ...
    //材质路径
    const string MaterialPath = "Assets/Art/Materials/UIOutline.mat";
    ...
    {
        Refresh();//由于页面为动态添加canvas，在Awake后刷新
    }
    ...
    public void Refresh()
    {
        if (graphic != null)
        {
            if (graphic.material == null || graphic.material.shader.name != "UI/Outline")
            {
                LoadOutlineMat();
            }
        }
