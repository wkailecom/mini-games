    ...
    public class AtlasCutting : UnityEditor.Editor
    {
        ...
        public static void DealPng(string assetPath)
        {
            ...
            for (int i = 1; i < assets2.Length; i++)
            {
                ...
                {
                    Debug.LogError($"{assets2[i].name} 图片类型错误，Texture Type 应为 Sprite(2D and UI)，Sprite Mode 应为 Multiple，Read/Write 应勾选，再点击 Sprite Editor 设置 Slice，图片压缩 None 质量为最佳。");
                }
            }
        }
    }
