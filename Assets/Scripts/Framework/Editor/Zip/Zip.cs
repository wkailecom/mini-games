...
public class ZipUtil
{
	...
	public static void Zip(bool isKeepDirectoryPathInArchive, string zipFileName, params string[] files)
	{
#if UNITY_EDITOR || UNITY_STANDALONE_WIN || UNITY_STANDALONE_OSX || UNITY_STANDALONE_LINUX
		string path = Path.GetDirectoryName(zipFileName);
		Directory.CreateDirectory(path);
		...
		using (ZipFile zip = new ZipFile())
		{
			foreach (string file in files)
			{
				string directoryPathInArchive = isKeepDirectoryPathInArchive ? Path.GetDirectoryName(file) : string.Empty;
