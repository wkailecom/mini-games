...
namespace ExcelConverter.DataTmpl
{
    public static class DataTmplCreator
    {
            ...
            ICell[] InAttrTypes, ICell[] InAttrNames, ICell[] InAttrComment = null)
        {
            if (null == InAttrTypes)
            {
                Log.Error("[DataTmplCreator]    Attribute type can not be null.");
                return;
            }

            if (null == InAttrNames)
            {
                Log.Error("[DataTmplCreator]    Attribute name can not be null.");
                return;
            }
            ...
            {
                Log.Error("[DataTmplCreator]    The number of attribute types and names must be the same.");
                return;
            }

            if (InAttrComment != null && attrLen != InAttrComment.Length)
            {
                Log.Error("[DataTmplCreator]    Attribute comments can be null or the number of comments and names must be the same.");
                return;
            }
