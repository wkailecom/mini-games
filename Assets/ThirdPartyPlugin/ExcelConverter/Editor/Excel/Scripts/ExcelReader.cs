                    ...
                    using (FileStream stream = info.Open(FileMode.Open, FileAccess.Read, FileShare.ReadWrite))
                    {
                        ...
                        if (sheetCount > 1)
                        {
                            for (int i = 0; i < sheetCount; i++)
                            {
                                ...
                                merge = sheet.SheetName.Contains("Sheet") || sheet.SheetName.Contains("工作表");
                                if (!merge) break;
                            }
                        }

                        List<ICell[]> content = new List<ICell[]>();
                        for (int i = 0; i < sheetCount; i++)
                        {
                            ISheet sheet = book.GetSheetAt(i);
                            ...
                            if (colCount > 0)
                            {
                                ...
                                if (merge && i != sheetCount - 1)//合并
                                    continue;

                                ExcelData data = new ExcelData
                                {
                                    SheetName = sheet.SheetName.Contains("Sheet") || sheet.SheetName.Contains("工作表")
