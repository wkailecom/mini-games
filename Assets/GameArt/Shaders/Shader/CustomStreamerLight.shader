Shader"Custom/StreamerLight" {
    Properties {
        ...
        _LightColor ("LightColor", Color) = (1,1,1,1)	//扫光颜色
    ...
    SubShader {
        ...
        pass {
            ...
            float inFlash(float angle, float2 uv, float xLength, int interval, int beginTime, float offX, float loopTime) {
                ...
                //返回颜色 = 纯白色 * 亮度
                float4 col = _LightColor * brightness;
                return brightness;
            }
            ...
            float4 frag(v2f i) : COLOR {
                ...
                //传进i.uv等参数，得到亮度值
                float tmpBrightness;
                tmpBrightness = inFlash(_Angle, i.screenPos.xy, _Width, _Interval, _BeginTime, _OffestX, _LoopTime);

                //图像区域，判定设置为 颜色的A > 0.5,输出为材质颜色+光亮值
                if (texCol.w > 0.5)
        outp = i.color + _LightColor * tmpBrightness;
